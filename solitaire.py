...
# mat size where we'll place the card on
MAT_PERCENT_OVERSIZE = 1.25
MAT_WIDTH = int(CARD_WIDTH * MAT_PERCENT_OVERSIZE)
MAT_HEIGHT = int(CARD_HEIGHT * MAT_PERCENT_OVERSIZE)

# size of gap between card mats
...
# Constants thaht represent "what pile is what" for the game
PILE_COUNT = 13
...
class Card(arcade.Sprite):
    ...
    def __init__(self, suite, value, scale=1):
        """ Card constructor """

        # Attributes for suite and value
        self.suite = suite
        self.value = value

        # Image to use for the sprite when face up
        self.image_file_name = f":resources:images/cards/card{self.suite}{self.value}.png"
...
class MyGame(arcade.Window):
    ...
    def __init__(self):
        ...
        # Sprite list with all the cards, no mater what pile they are in
        self.card_list = None
    ...
    def setup(self):
        ...
        # Create every card
        for card_suite in CARD_SUITS:
            for card_value in CARD_VALUES:
                card = Card(card_suite, card_value, CARD_SCALE)
    ...
    def remove_card_from_pile(self, card):
        """ Remove card from whatever pile was in """
    ...
    def on_mouse_release(self, x: float, y: float, button: int, modifiers: int):
        ...
        # See if we are in contact with closest pile
        ...
        if reset_position:
            # Where-over we were dropped, it wasn't valid. Reset the each card's position
